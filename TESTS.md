...
| Disposal floor (vol-scaled Normal @85%) | ✅ | Walk-forward calibration: **85.0% across all players** (bang on), but **82.5% on the shown floor>=10 slate** — steady every season, confirmed live (R16–R22 shown mostly 81–84%). High-floor picks ran ~2.5pts **optimistic** (disposal left tail heavier than Normal: tags, early subs). | `exp_floor_calib.py` |
| **Heavy-tail floor fix** (implemented, from R23) | ✅ | Adds a conditional haircut for high-usage players only: `z_eff = z(0.85) + 0.30·max(0,proj−15)/10` (`matchup.HEAVY_TAIL`, shared by page + scorecard). Beats a global z bump: shown slate → **~85%** (per-season 84.5–85.2, walk-forward 2022–26) while **keeping 99% of shown picks** (21,560 vs 21,738) and barely moving the already-calibrated all-players set — a global z=0.89 hit 85% but lost ~10% of picks and over-tightened all-players to 88%. Live from R23; earlier rounds keep their frozen (old) floors. | `exp_floor_z.py` |
...
| Disposals-against **by position type** — inside/outside mid split via CBA (the "FNFGainz" DvP idea) | ✅ | **Null — the fourth and final DvP granularity to die.** Taxonomy validated by eye (IM = avg CBA ≥40%: Neale/Newcombe/Daicos; OM ≤15%: Whitfield-style runners; rucks peeled off first; 12,838 classified games 2025–26). No position type's concession persists round-to-round (lag-1 autocorr +0.02…+0.08 ≈ noise); residual corrs all ≤ +0.09 (defenders the largest, r² <1%); selection gradient softest-vs-toughest only +2.1pts floor-hit (87.2% vs 85.1%) — same faint lean as line-level, not usable. **Nugget:** uncontested-ball conceded is the most persistent team trait tested (lag-1 +0.14; +0.12 season-relative — pressure is a real trait) but sub-threshold and it produces no tilt for outside mids (flat, non-monotonic). Caveat: many classic outside runners (Ash, Dale, Whitfield) classify as running *defenders* off half-back, so the OM bucket is small (n=655) — but the defender bucket (n=6,219) is equally null. | `exp_position_da.py` |
...
|---|:--:|---|---|
| **Market line as a projection INPUT** (blend toward the Sportsbet ladder's implied median) | ✅ | The first *disposal* MAE gain in the program: on R13 (294 priced players, ladders from git history) a 50/50 blend of our projection + vig-adjusted market median beat both alone — **3.40 vs our 3.47 (−2.1%)**. Market alone does *not* beat us (3.57; raw vig-inflated 3.86). **Replicated on R14** (207 priced players): 50/50 blend 3.54 vs our 3.62 (−2.2% raw; −3.3% on the OOS subset). **Two rounds, both positive — the most robust accuracy result in the program.** Snapshots capture `d_ladder` so it keeps validating forward; worth moving toward production (blend the projection toward the market median). Tension to manage: market-anchored floors shrink the value tints toward zero by construction. | `exp_market.py` |
