...
def report_md(path=LIVE_MD):
    ...
    rows = ["| Round | Games | Floor-hit (shown) | Value picks | Value ROI |",
            "|------:|:-----:|:-----------------:|:-----------:|:---------:|"]
    for _, r in comp.iterrows():
        roi = r.get("value_roi")
        roi_s = f"{roi*100:+.1f}%" if pd.notna(roi) else "-"
        rows.append(f"| R{int(r['round'])} | {int(r['games_total'])} | "
                    f"{r['shown_hit']*100:.1f}% | {int(r['value_n'])} | {roi_s} |")
    sn = comp["shown_n"].sum()
    hit = (comp["shown_hit"] * comp["shown_n"]).sum() / sn if sn else float("nan")
    vn = int(comp["value_n"].sum())
    roi = comp["value_profit"].sum() / vn if vn else float("nan")
    nr = len(comp); rl = f"{nr} round" + ("s" if nr != 1 else "")
    rows.append(f"| **Running total** | **{rl}** | **{hit*100:.1f}%** | "
                f"**{vn}** | **{roi*100:+.1f}%** |")
