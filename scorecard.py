...
def report_md(path=LIVE_MD):
    ...
    for _, r in comp.iterrows():
        # NaN value_n = a reconstructed (floors-only) round: the pre-game odds
        # were never captured, so there are no value picks to report — not zero.
        recon = pd.isna(r["value_n"])
        recon_any = recon_any or recon
        vp = "—†" if recon else str(int(r["value_n"]))
        rows.append(f"| R{int(r['round'])} | {int(r['games_total'])} | "
                    f"{r['shown_hit']*100:.1f}% | {vp} |")
    sn = comp["shown_n"].sum()
    hit = (comp["shown_hit"] * comp["shown_n"]).sum() / sn if sn else float("nan")
    vn = int(comp["value_n"].sum())                                 # NaN rounds skipped
    ...
    foot = ("\n\n*† Floor-only reconstruction — the app was offline that week so the "
            "pre-game odds weren't captured. The floor numbers are genuine (they're "
            "deterministic from prior form); there are simply no value picks for those "
            "rounds.*") if recon_any else ""
    block = ("<!-- SCORECARD:START (auto-generated by `scorecard.py report-md`; do not edit by hand) -->\n\n"
             + "\n".join(rows) + f"\n\n*Last updated: {today} — {rl} complete.*" + foot + "\n\n"
             + "<!-- SCORECARD:END -->")
