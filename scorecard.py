...
def report_md(path=LIVE_MD):
    ...
    foot = ("\n\n*† Floor-only round — no live pre-game odds were captured, so there are "
            "no value picks. The floor numbers are genuine (they're deterministic from "
            "prior form, independent of the market).*") if recon_any else ""
